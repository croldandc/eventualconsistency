...
%	\item \label{prop_sent} If $\cid_l \in \dom\inserver$ and $\cid_l \in \dom\outserver$ then ${\pushbuffer}_l \ \triangleleft \ {\tsent}_t$  $\tsent_l$ = ${\tsent}_h \cdot {\tsent}_t$ and and $ \ireduce{\igetdeltas{\inserver(l)}} \triangleleft \ {\tsent}_h$
	...
	\begin{itemize}
	 ...
	 $ \ireduce{\igetdeltas{\inserver(\cid_l)}\cdot {\pushbuffer}_l} \triangleleft \ {\tsent}_l$; or
	   \item  $\cid_l \not\in\dom\inserver$...
	\end{itemize}
	...
	\item  \label{prop_size_buffersent} $m_l = n_l$ \chnote{Habría que ponerle otro font a la del abstracto} \henote{le llame m al abst}

\item \label{prop_inclient} if $\cid_l \in \dom{\outserver}$ then:
    \begin{itemize}
%    \item  ${\inclient}_l = \epsilon$ and $\treceivebuffer = 0$; or  
    \item  ${\inclient}_l \cdot \outserver(\cid_l)  = \agssegpair[{\astate[']}]\cdot\agssegpairi[1] \cdots \agssegpairi[h]$
     % $\ireduce{\adelta_1\cdots\adelta_h}\triangleleft  \queuemessage[\tknown..|\queuemessage| - 1]$ 
     and  
     $\iapply{\astate[']}{\ireduce{\adelta[_1]\cdots\adelta[_{h}]}}\triangleleft \queuemessage )$; or 
     
     
     \item ${\inclient}_l = \agssegpairi[1] \cdots \agssegpairi[h']$, $\outserver(\cid_l) = \agssegpairi[h'+1] \cdots \agssegpairi[h]$; 
     \linebreak
      $\ireduce{\adelta_1\cdots\adelta_{h'}}\triangleleft  \queuemessage[\tknown_l..\tknown_l+x-1])$; and $\ireduce{\adelta_{h'+1}\cdots\adelta_h}\triangleleft 
       \queuemessage[\tknown_l+x..|\queuemessage| - 1])$; and $x \leq \tknown_l$.
     
%     \item
%     ${\inclient}_l \cdot \outserver(\cid_l) = \agssegpairi[1] \cdots \agssegpairi[h]$ and $\ireduce{\adelta_1\cdots\adelta_h}\triangleleft  \queuemessage[\tknown_l..|\queuemessage| - 1])$;
    \end{itemize}
...
\item \label{prop_inclient} if $\cid_l \in \dom{\outserver}$ then:
    ...
    \item  ${\inclient}_l \cdot \outserver(\cid_l)  = \agssegpair[{\astate[']}]\cdot\agssegpairi[1] \cdots \agssegpairi[h]$
     ...
     and  
     $\iapply{\astate[']}{\ireduce{\adelta[_1]\cdots\adelta[_{h}]}}\triangleleft \queuemessage )$; or 
     \item ${\inclient}_l \cdot \outserver(\cid_l) = \agssegpairi[1] \cdots \agssegpairi[h]$ and $\ireduce{\adelta_1\cdots\adelta_h}\triangleleft  \queuemessage[\tknown_l..|\queuemessage| - 1])$;
     \end{itemize}
	...
	\henote{Agregar flatten por todos lados}
...
%						Then, this property holds.
						...
						\item $\cid\in\dom\inserver$: \henote{Completar}
						\end{itemize}
						...
						$\implements{\isystemterm}{\systemterm}$, the following holds
						\begin{itemize}
							\item $\transactionbuffer \triangleleft{\ttransactionbuffer}$ by property \ref{prop_transactions};  \henote{Cambiar por macro}
					...
					\[
  						...
  						{\ireduce{\igetdeltas{\pending}\cdot{\pushbuffer}\cdot \transactionbuffer}\triangleleft \flatten{\tpending\cdot [\ttransactionbuffer]}}  
					\]
					We have used that $\flatten{\tpending\cdot [\ttransactionbuffer]}$ = $\flatten{\tpending} \cdot \ttransactionbuffer$, besides,
						
						\item[\ref{prop_size_buffersent}.] It trivially holds since in both cases is $n_l + 1$.
						\item[{--}] The remaining properties straightforwardly hold.
...
\item {\bf rule \ruleName{receive}}. Then, $\isystemterm  \arroi{\tau} \isystemterm '$ with 
				\[\begin{array}{r@{=}l}
				  ...
				  \end{array}
				\]
				\henote{la macro para i, por todos lados}
	...
	\henote{ver la definicion de N, el inc}
				...
				\henote{Esta nota es vieja o vigente? Me parece que ya hablamos}.
