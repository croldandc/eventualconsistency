...
\begin{enumerate}
	...
	\item \label{prop_sent} If $\cid \in \inserver \ \land \  \cid \in \outserver \ $then $(\tsent_l$ = ${\tsent}_h \cdot {\tsent}_t \ \land \ {\pushbuffer}_l \ \triangleleft \ {\tsent}_t \ \land \ \ireduce{\igetdeltas{\inserver(l)}} \triangleleft \ {\tsent}_h) \ $
	\item \label{prop_pending} $\pending_l = \aroundtuplei[1]\ldots\aroundtuplei[h]$ and $\ireduce{\adelta_1\cdots\adelta_h\cdot {\pushbuffer}_l} \triangleleft \flatten{\tpending_l}$
	\item \label{prop_size_buffersent} $n_l = n_l$ \chnote{Habr�a que ponerle otro font a la del abstracto}
\item \label{prop_inclient} $(if \ l \in Dom(\outserver) \ then ({\inclient}_l = \epsilon \implies \treceivebuffer = 0))\  \lor \ $ 
    					        $({\inclient}_l \cdot \outserver(l)  = \agssegpair[{\astate[']}]\cdot\agssegpairi[1] \cdots \agssegpairi[h] \land \ireduce{\adelta_1\cdots\adelta_h}\triangleleft  \queuemessage[\tknown..|\queuemessage| - 1]\ \land \ \iapply{\astate[']}{\ireduce{\adelta[_1]\cdots\adelta[_{h}]}}\triangleleft \queuemessage ) \ \lor \ $ 
					        $({\inclient}_l \cdot \outserver(l) = \agssegpairi[1] \cdots \agssegpairi[h] \land \ireduce{\adelta_1\cdots\adelta_h}\triangleleft  \queuemessage[\tknown_l..|\queuemessage| - 1])$
...
\item {\bf rule \ruleName{push}}. Then, $\isystemterm  \arroi{\pushtran} \isystemterm '$ with 
				...
				by checking the properties in \defref{def:implementation}.			
					\begin{enumerate}
						...
						\item[\ref{prop_sent}.] The hypothesis is that $\tsent$ = ${\tsent}_h \cdot {\tsent}_t $ and $ {\pushbuffer}_l\ \triangleleft \ {\tsent}_t$  and $\ireduce{\igetdeltas{\inserver(l)}} \triangleleft \ {\tsent}_h$. Now, $\tsent$ and $\pushbuffer$ have changed, in particular ${\tsent}_t $ has a new block. $\inserver$ has not changed. We should build the following proof for $\ireduce{{\pushbuffer}\cdot \transactionbuffer}\triangleleft {\tsent}_t\cdot\ttransactionbuffer \ $.
						\[
					...
					\[
  						...
  						{\ireduce{\igetdeltas{\pending}\cdot{\pushbuffer}\cdot \transactionbuffer}\triangleleft \flatten{\tpending\cdot [\ttransactionbuffer]}}  
					\]
					We have used that $\flatten{\tpending\cdot [\ttransactionbuffer]}$ = $\flatten{\tpending} \cdot \ttransactionbuffer$, besides, since 
						$\implements{\isystemterm}{\systemterm}$, the following holds
						\begin{itemize}
							\item $\transactionbuffer \triangleleft{\ttransactionbuffer}$ by property \ref{prop_transactions};
		...
		\ \bigpar\ \iclient[''] \\
				...
				by checking the properties in \defref{def:implementation}.			
						...
						\begin{itemize}
							...
							\item[\ref{prop_sent}.] We have moved the ${\pushbuffer}$ to $\inserver(i)$, leaving in the push buffer, $\emptydelta$. Furthermore, ${\tsent}_t$ is $\epsilon$. By \triangemptydelta this part of the property holds. 	
					...
					\]
							...
							\item[\ref{prop_pending}.] We have to prove that $\pending[']_i = \aroundtuplei[0]\ldots\aroundtuplei[h]\aroundtuplei[p]$ and $\ireduce{\adelta_0\cdots\adelta_h\cdot\adelta_p\cdot\emptydelta} \triangleleft \flatten{\tpending_i}$. It is easy to see that it is the same relation that our hypothesis, since $\adelta_p$ was moved to the pending sequence. Therefore this property also holds.
						\item[{--}] The remaining properties straightforwardly hold.
...
\item {\bf rule \ruleName{receive}}. Then, $\isystemterm  \arroi{\tau} \isystemterm '$ with 
	...
	\item {\bf rule \ruleName{pull-1}}. Then, $\isystemterm  \arroi{\pulltran} \isystemterm '$ with 
				...
				\[\begin{array}{r@{=}l}
					\systemterm^{h} &  \tsystem{\tclienti{\tpullins}{\tknown}{\tpending}{\ttransactionbuffer}{\tsent}{\treceivebuffer + h}}[{\systemterm['']}] \\
				  \end{array}		
				\]
			
				
				$h$ represents the amount of internal transition that performed by client $\cid$. By \propref{prop_inclient}, we know that 
				$\ireduce{\adelta[_1]\cdots\adelta[_m]}\triangleleft \queuemessage[\tknown_i .. |\queuemessage| - 1]$ then $\exists \ t \in \ \nat, 0 \leq t \leq m$ such that $\ireduce{\adelta[_1]\cdots\adelta[_n]}\triangleleft \queuemessage[\tknown_i .. \tknown_i + h]$
				
				Now by inspection of rules in \figref{}, 
				$\systemterm^{h} \arroi{\pulltran} \systemterm[']$ with
				\[\begin{array}{r@{=}l}
					\systemterm['] &  \tsystem{\tclienti{\tprogram}{\tknown_i+h}{\tpending_i \setminus \queuemessage[\tknown_i .. \tknown_i + \treceivebuffer_i + h {\ -1}]}{\ttransactionbuffer}{\tsent}{0}}[{\systemterm[''']}]	
				  \end{array}		
				\]
				...
				\chnote{esta oka la regla pull1? el in del cliente no entiendo porque tiene el delta y la f, para que la usas?}
					...
					\item[\ref{prop_inclient}.] $\inclient_l$ = $\epsilon$ and $\treceivebuffer_l$ = 0, then this property is hold.
					...
					\]
						...
						\item[\ref{prop_pending}.] We have to prove that $filter\ (\geq \amxrf[_n](\cid)) \ \aseqround \triangleleft \flatten{\tpending \setminus \queuemessage[\tknown .. \tknown + \treceivebuffer -1]}$ (PENDIENTE)
				...
				by checking the properties in \defref{def:implementation}.	
					\begin{enumerate}
						\item[\ref{prop_sent}.] $\inserver$ has not rounds,i.e., is $\epsilon$. After $h$ transition in the abstract model, ${\tsent}_h$ is $\epsilon$, using \triangemptydelta we can see that this property holds.
					      	\item[\ref{prop_inclient}.] We build the following proof for 
						$\ireduce{\adelta_1\cdots\adelta_h\cdot\adelta} \triangleleft \ \queuemessage$
						\[
						  \mathrulean{\lemref{lemma:reduce}}
						  {
								\ireduce{\adelta_1\cdots\adelta_h}\triangleleft  \queuemessage[\tknown_i .. \tknown_i + t - 1]
								\quad 
								\delta \triangleleft  \queuemessage[t .. + |\queuemessage|- 1]					
							}
  						{\ireduce{\adelta_1\cdots\adelta_h\cdot\adelta} \triangleleft \ \queuemessage}  
					\]
					Note that $t$ is the length of $\queuemessage$ before the pull transition.
						\item[\ref{prop_stateserver}.] We have to prove that 
						$\iapply{\astate}{\ireduce{\adelta[']_1\cdots\adelta[']_h}} \triangleleft \queuemessage\cdot [\tsenthead_1]\cdots[\tsenthead_h] \ $. The proof is immediate by \lemref{lemma:storechanged}.
