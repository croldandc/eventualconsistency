...
\item {\bf rule \ruleName{receive}}. Then, $\isystemterm  \arroi{\tau} \isystemterm '$ with 
				...
				Since $\implements{\isystemterm}{\systemterm}$, the property \refprop{prop_inclient} holds. We have two cases:
				\begin{itemize}
					\item  $\outserver(\cid) = \agssegpairi[h'+1] \cdots \agssegpairi[h]$, then $\ireduce{\adelta_{h'+1}\cdots\adelta_h}\triangleleft \
       \flatten{\queuemessage[\tknown+\treceivebuffer..|\queuemessage| - 1]}$. By \triangreduce \ we know that $\adelta_{h'+1}\triangleleft \flatten{\queuemessage[\tknown+\treceivebuffer .. m - 1]}$ with $m < |\queuemessage| - \tknown- \treceivebuffer$. Hence, 
				...
				\begin{itemize}
					...
					\item[\ref{prop_inclient}.] ${\inclient} = \agssegpairi[1] \cdots \agssegpairi[h']\cdot\adelta_{h'+1}$ and $\outserver(\cid) = \agssegpairi[h'+2] \cdots \agssegpairi[h]$. We prove that
					\[\ireduce{\adelta_1\cdots\adelta_{h'+1}}\triangleleft \flatten{ \queuemessage[\tknown..\tknown+\treceivebuffer+m-1]}\]
						...
						\[
						  \mathrulean{\lemref{lemma:reduce}}
							...
							\quad
							\adelta_{h'+1} \triangleleft \flatten{ \queuemessage[\tknown+\treceivebuffer..m-1]}}
  						       {\ireduce{\adelta_1\cdots\adelta_{h'+1}}\triangleleft \flatten{ \queuemessage[\tknown..\tknown+\treceivebuffer+m-1]}}  
						\]
