...
We remark that  $\mathcal{R}^{-1}$ is not a simulation essentially  because the implementation cannot mimic  the 
behaviour of a client that have performed two 
consecutive \pushcmd\ commands without sending the first block: in \gsp\, the global store is still able to interleave the two 
blocks with updates arriving from other clients. Differently the two blocks will be treated as atomic in \igsp\  
