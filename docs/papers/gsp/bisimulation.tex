...
\item {\bf rule \ruleName{push}}. Then, $\isystemterm  \arroi{\pushtran} \isystemterm '$ with 
				...
				by checking the properties in \defref{def:implementation}.			
					...
					\[
  						...
  						{\ireduce{\igetdeltas{\pending}\cdot{\pushbuffer}\cdot \transactionbuffer}\triangleleft \flatten{\tpending\cdot [\ttransactionbuffer]}}  
					\]
					We have used that $\flatten{\tpending\cdot [\ttransactionbuffer]}$ = $\flatten{\tpending} \cdot \ttransactionbuffer$, besides, we use \propref{prop_transactions}.
	...
	\item {\bf rule \ruleName{pull-2}}. Then, $\isystemterm  \arroi{\tau} \isystemterm '$ with 
		...
		\chnote{por que ocurre lo del inserver en pull 2?}
			...
			\item {\bf rule \ruleName{accept-conn}}. Then, $\isystemterm  \arroi{\tau} \isystemterm '$ with 
				...
				by checking the properties in \defref{def:implementation}.	
				\begin{enumerate}
					\item[\ref{prop_send}.] When a connection is accepted then there are two cases for client i.
						\begin{itemize}
							\item The client is a new one, hence, this client has their queues empty in the abstract model. In particular, $\tsent$.
							\item The client has worked but it got lost its connection. When it happens, in the  
						\end{itemize}

					
%					It is immediately by $\triangreduceemp$.We know that $\state \triangleleft \queuemessage}$, besides by \propref{prop_outserver2}$\state = \iapply{\astate[']}{\ireduce{\adelta_0\cdots\adelta_h}$ .
%					\item[\ref{prop_outserver2}.] We know that $\state \triangleleft \queuemessage}$, besides by \propref{prop_outserver2}$\state = \iapply{\astate[']}{\ireduce{\adelta_0\cdots\adelta_h}$ .
