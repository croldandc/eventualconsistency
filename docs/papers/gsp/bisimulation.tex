...
\item {\bf rule \ruleName{receive}}. Then, $\isystemterm  \arroi{\tau} \isystemterm '$ with 
       ...
       \flatten{\queuemessage[\tknown+\treceivebuffer..|\queuemessage| - 1]}$. By \triangreduce \ we know that $\adelta_{h'+1}\triangleleft \flatten{\queuemessage[\tknown+\treceivebuffer .. \tknown+\treceivebuffer +m - 1]}$ with $m < |\queuemessage| - \tknown- \treceivebuffer$. Hence, 
			...
			%		\flatten{ \queuemessage[\tknown+\treceivebuffer..m-1]}$. Then,
					...
					\item  $\outserver(\cid) = \agssegpair[{\astate[']}]\cdot\agssegpairi[1] \cdots \agssegpairi[h]$, then $\iapply{\astate[']}{\ireduce{\adelta[_1]\cdots\adelta[_{h}]}}\triangleleft \flatten{\queuemessage}$. 
				...
				\[\begin{array}{r@{=}l}
					\systemterm['] &  \tsystem{\tclienti{\tprogram}{\tknown+r+r}{\tpending_i \setminus \queuemessage[\tknown.. \tknown+ \treceivebuffer +  r + m  {\ -1}]}{\ttransactionbuffer}{\tsent}{0}}[{\systemterm[''']}]	
				  \end{array}		
				\]
