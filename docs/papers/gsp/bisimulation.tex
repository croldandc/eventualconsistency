				...
				by checking the properties in \defref{def:implementation}.			
					...
					We have used that $\flatten{\tpending\cdot [\ttransactionbuffer]}$ = $\flatten{\tpending} \cdot \ttransactionbuffer$, besides, we use \propref{prop_transactions}.
					  \item[\ref{prop_size_buffersent}.] $n_l = n_l$, both have been incremented by one, so that this property is hold.
						...
						\item[{--}] The remaining properties straightforwardly hold.
						\chnote{IMPORTANTE: Falta la relacion que explica que pasa con el send del abstracto. Cuando termine todas las reglas quizas entienda que hacer.}			
...
\item {\bf rule \ruleName{receive}}. Then, $\isystemterm  \arroi{\tau} \isystemterm '$ with 
				...
				by checking the properties in \defref{def:implementation}.			
						...
						\begin{itemize}
							\item[\ref{prop_inclient}.] By \propref{prop_outserver}, we know that the firt element in $\outserver(i)$ = $\queuemessage[\tknown_l+\treceivebuffer_l]$ when $m$ = 1. Let $\adelta_{h+1}$ be a delta object associated to $\aseg$. Now, we should build the following proof for 
					...
					\[
  						...
  						{\ireduce{\adelta_0\cdots\adelta_{h+1}}\triangleleft  \queuemessage[\tknown_l..\tknown_l+\treceivebuffer_l]}  
					\]
					We used that $\queuemessage[\tknown_l..\tknown_l+\treceivebuffer_l]$ = $\queuemessage[\tknown_l..\tknown_l - 1] \cdot \queuemessage[\tknown_l + \treceivebuffer_l]$. Furthermore, we used the hypothesis from \propref{prop_inclient} that states $\ireduce{\adelta_0\cdots\adelta_{h}}\triangleleft  \queuemessage[\tknown_l..\tknown_l+\treceivebuffer_l - 1]$.
							\item[\ref{prop_outserver}.] \propref{prop_outserver} states that for all natural $m$, such that $0 \leq m \leq |\outserver(i)|$ then $\outserver(i)[m] \triangleleft \queuemessage[\tknown + \treceivebuffer - 1 + m]$. Now, $|\outserver(i)|$ was decremented by one, then this property is hold. (Si valia para los menores a longitud outserver, entonces es trivial digamos ya que ahora vale para longitud outserver - 1) 
						\end{itemize}
...
\item {\bf rule \ruleName{confirm}}. Then, $\isystemterm  \arroi{\confirmedtran} \isystemterm '$ with 
	...
	\item {\bf rule \ruleName{batch}}. Then, $\isystemterm  \arroi{\tau} \isystemterm '$ with 
				...
				by checking the properties in \defref{def:implementation}.	
					\begin{enumerate}
							\item[\ref{prop_stateserver}.] We should build the following proof for 
						$\iapply{\astate}{\ireduce{\adelta[_1]\cdots\adelta[_h]}} \triangleleft \queuemessage\cdot [\tsenthead_1]\cdots[\tsenthead_h] \ $
