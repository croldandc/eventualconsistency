...
The  first three conditions are self-explanatory. Condition (\ref{prop_pending})  states that  pending blocks $\flatten{\tpending_l}$ in the abstract model 
corresponds either to sent rounds that are still pending, \ie, within $\pending_l$, or to blocks ready to 
...
then  all completed blocks  in  $\tsent_l$  corresponds to rounds 
that are already in the input buffer $\inserver(\cid_l)$ of the store or ready to sent in  ${\pushbuffer}_l$.
Condition (\ref{prop_inclient}) establishes the relations between the received messages in both models. Basically, 
when there are no segments for the client (${\inclient}_l \cdot \outserver(\cid_l)  = \epsilon$), then the local replica 
is complete ($i$). When the first received segment is a delta object ($ii$),
 the client's input buffer $\inclient_l$ corresponds to the received messages in $\queuemessage[\tknown_l..\tknown_l+\treceivebuffer_l-1]$
and the the store's output buffer $\outserver(\cid_l)$ contains the remaining updates in the store, 
\ie $\queuemessage[\tknown_l+\treceivebuffer_l..|\queuemessage| - 1]$. The remaining items consider
the cases in which the first segment contains a state. If the segment is already 
...
first segment is still on the output buffer of the store ($iv$), the state $\astate[']$  corresponds to a
 prefix that contains all  updates in $\queuemessage$ already known by the client, 
 \ie, $t \geq \tknown_l+\treceivebuffer_l $.    
 
 The last condition (\ref{prop_deltas}) states that for any segment $\agssegpair$  sent by the 
 store, $\adelta$ corresponds to a contiguous sequence of updates in $\queuemessage$. 
 Moreover, all confirmed rounds  (\ie, $\irounds\leq\amxrf(\cid_l)$) are already in $\queuemessage$
 within the prefix delimited by $\adelta$.
