...
\item \label{prop_inclient} if $\cid_l \in \dom{\outserver}$ then either
   ...
   % \item  ${\inclient}_l \cdot \outserver(\cid_l)  = \agssegpair[{\astate[']}]\cdot\agssegpairi[1] \cdots \agssegpairi[h]$
    ...
    \end{enumerate}	
        ...
        \item \label{prop_deltas} for all $\agssegpair\in\inclient_l\cdot\outserver(\cid_l)$ (similarly, $\agsprefpair\in\inclient_l\cdot\outserver(\cid_l)$) 
%          \begin{enumerate}[i.]
...
when
 ...
 We rely on a standard %notion of simulation 
