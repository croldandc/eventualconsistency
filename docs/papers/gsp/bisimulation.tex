...
%				Then by rule (\textsc{read}), $\isystemterm' = \clientr{\update{P}{x}{v}}{\stateclient}\ \bigpar\ \tilde{C} \ \bigpar\ \ \queuemessage_C$. We will prove that $(\systemterm',\isystemterm') \ \in \ \mathcal{R}^{-1}$ if the ten properties introduced are keeping.				
				...
				Since $(\systemterm,\isystemterm)\in \mathcal{R}^{-1}$,
				...
				We  prove that $(\systemterm',\isystemterm') \ \in \ \mathcal{R}^{-1}$ by showing that $\implements{\isystemterm[']}{\systemterm[']}$ 
				by checking the properties in \defref{def:implementation}.			
					\begin{enumerate}
						...
						\begin{itemize}
							...
							\item $\igetdeltas{\pending}\cdot{\pushbuffer} \triangleleft \flatten{\tpending} \ $ by property \ref{prop_pending};
