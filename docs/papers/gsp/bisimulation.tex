...
\begin{itemize}
   ...
   \item{\bf n=k+1}. $\forall (\systemterm ,\isystemterm ) \ \in \ \mathcal{R}^{-1}$
			...
			\begin{itemize}
				...
				Then by rule (\textsc{read}), $\isystemterm' = \clientr{\update{P}{x}{v}}{\stateclient}\ \bigpar\ \tilde{C} \ \bigpar\ \ \queuemessage_C$. We will prove that $(\systemterm',\isystemterm') \ \in \ \mathcal{R}^{-1}$ if the ten properties introduced are keeping.				
					\begin{enumerate}
						\item The only terms that have changed are the programs from i-esimo client respectively. In particular, the client programs are equals, then, we should show that the value got is the same in both systems, i.e., 
						$\rvalue{r}{\flatten {\queuemessage[0..\tknown-1] \cdot \tpending} \cdot \ttransactionbuffer}$ 
						= 
						$\iread{r}{\iapply{\known}{{\igetdeltas\pending}\cdot{\pushbuffer}\cdot{\transactionbuffer}}}$.
						Then by \triangread \ It is worth when:
						$\flatten {\queuemessage[0..\tknown-1] \cdot \tpending} \cdot \ttransactionbuffer$
						$\triangleleft$
						$\iapply{\known}{{\igetdeltas\pending}\cdot{\pushbuffer}\cdot{\transactionbuffer}}$. By \triangapply \ It’s worth if the following relations are satisfied:
						
						\begin{itemize}
							\item $\known \triangleleft \flatten {\queuemessage[0..\tknown-1]}$; but It’s worth by Property \ref{prop_state_known}
							\item $\igetdeltas{\pending}\cdot{\pushbuffer} \triangleleft \flatten{\tpending} \ $; It finally is worth by \ref{prop_pending}
							\item $\transactionbuffer \triangleleft{\ttransactionbuffer}$; It finally is worth by Property \ref{prop_transactions}
						\end{itemize}
