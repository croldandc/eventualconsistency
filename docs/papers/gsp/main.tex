...
\begin{proof}
We have to prove that $\mathcal{R}$ satisfies the nine points of its definition:
...
\begin{enumerate}
	...
	\item $\ E_{i}.\transactionbuffer = [\ttransactionbuffer_i]$
...
\begin{itemize}
   ...
   \item{\bf n=k+1}. $\forall (A,C) \ \in \ \mathcal{R}$
			...
			\begin{itemize}
				\item {\bf rule (\textsc{T-READ})}. Then $A = \tsystem{\tclienti{\treadins{x}{r}}{\tknown}{\tpending}{\ttransactionbuffer}{\tsent}{\treceivebuffer}\bigpar\ \tilde{A}}{\queuemessage_A}$ and $A' = \tsystem{\tclienti{\update{\tprogram}{x}{v}}{\tknown}{\tpending}{\ttransactionbuffer}{\tsent}{\treceivebuffer}\bigpar\ \tilde{A}}{\queuemessage_A}$. As $(A,C)$ belongs $\mathcal{R}$ then $C$ must be $\{\readp{r}{\curstate{\known}{\pending}{\pushbuffer}{\transactionbuffer}}\}_i \bigpar\ \tilde{C} \ \bigpar\ \ \queuemessage_C$, so that, read transition is the only one that $C$ could perform.
				Then, $C' = \clientr{\update{P}{x}{v}}{\stateclient}\ \bigpar\ \tilde{C} \ \bigpar\ \ \queuemessage_C$. If $(A',C') \ \in \ \mathcal{R}$ then we will check the previous nine properties:
