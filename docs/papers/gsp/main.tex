...
\newcommand{\startsynctran}{\textit{start-sync}}
\newcommand{\finishsynctran}{\textit{end-sync}}
...
\newcommand{\environmenttran}[7]{\{#1 , #2, #3, #4, #5, #6, #7\}}
...
Conversely, assume $(a,b) \in\ (\textsc{\small{SO}}_R \ \cup \ \textsc{\small{VIS}} ) ^{k+1}$ = $(\textsc{\small{SO}}_R \ \cup \ \textsc{\small{VIS}} ) ^{k} \circ (\textsc{\small{SO}}_R \ \cup \ \textsc{\small{VIS}})$. Then there is a vertice $c \in \verticesets$ such
...
Assume $(a,b) \in\ (\textsc{\small{SO}}_R \ \cup \ \textsc{\small{VIS}} ) ^{k+1} $ = $(\textsc{\small{SO}}_R \ \cup \ \textsc{\small{VIS}} ) ^{k} \circ (\textsc{\small{SO}}_R \ \cup \ \textsc{\small{VIS}})$. Then there is a vertice $c \in \verticesets$ such
that $(a,c) \in\ (\textsc{\small{SO}}_R \ \cup \ \textsc{\small{VIS}} ) ^{k}$ and $(c, b) \in\ (\textsc{\small{SO}}_R \ \cup \ \textsc{\small{VIS}} )$.
...
\paragraph{Transactions} relates vertices from a client to set of naturals. 

\[
    \begin{array}{l@{\quad}r@{\;::=\;}l}
			 (\textsc{environment}) & \environmentterm &  \environmenttran{\systemterm}{\op}{\so}{\vis}{\arb}{\rb}{\tx} \\
			 (\textsc{transactions}) & \tx &  \emptyset  \ | \ v^i \mapsto \mathcal{N},\tx  \\
...
dom(\tx)}{\environmenttran{\systemterm}{\op}{\so}{\vis}{\arb}{\rb}{\tx} \arroi{\startsynctran} \environmenttran{\systemterm'}{\op}{\so}{\vis}{\arb}{\rb}{\tx}}

 \\[35pt]

\mathrule{e-end-sync}{\systemterm \arroi{\finishsynctran(u^{[v]})} \systemterm' \qquad [v^i] \in 
dom(\tx)}{\environmenttran{\systemterm}{\op}{\so}{\vis}{\arb}{\rb}{v^i \mapsto \mathcal{N},\tx} \arroi{\finishsynctran([v^i])} \environmenttran{\systemterm'}{\op}{\so}{\vis}{\arb}{\rb'}{\tx}}
