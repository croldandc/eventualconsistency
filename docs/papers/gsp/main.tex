...
The global state of a store and the local  replicas maintained by the clients are 
represented by sequences containing update operations. 
%For technical 
%convenience, we will decorate each update operations $u$ with the 
%identity $i$ of the client that issued it, written $u_i$.  
%
We  use $\ttransactionbuffer$, $\beta$ to denote sequences of updates and
$\ublock$ for an atomic block of updates performed by the client $i$. 
(We reserve the term 
transaction for a different notion addressed in \secref{sec:transactions}). 
We use  $\tpending$ and $\sigma$ 
for sequences of blocks.
Formally,
\[ \ttransactionbuffer, \beta := \epsilon \ |\ u\cdot\alpha 
    \qquad\qquad 
    \tpending,\sigma:= \epsilon\ |\ \ublock\cdot\tpending\]
where  $\epsilon$ is the empty sequence.
%  defined by the following grammar
% \[\tpending,\sigma ::= \epsilon \ |\  [\alpha] \cdot \tpending\]
...
\begin{itemize}
    ...
    %relative complement of $\tpending$ in $\ttransactionbuffer$, \ie,
     $\tpending \setminus \ttransactionbuffer = [u_i | u_i \in \tpending \wedge\ u_i\notin \ttransactionbuffer]$. 
     \henote{fix}
    \item Flatten: $\flatten{\epsilon} = \epsilon$, and 
    $\flatten{\ublock\cdot\tpending} = \ttransactionbuffer\cdot\flatten\tpending$.
    \item subsequence: $\ttransactionbuffer[j.. k]$ with $j,k\in\nat$.
    \item length: $\length\alpha$
\end{itemize}



 \begin{definition}[Global Sequence Protocol Language] 
The set of \tgspcalculus\ expressions is given by the following grammar:
 
  \[
    \begin{array}{l@{\quad}r@{}l}
			 (\textsc{store}) & \queuemessage \::=\ &  \tpending \\
			 (\textsc{program}) & \tprogram \::=\ & \tupdateins \;|\; \treadins{x}{r} \;|\; \tpullins \;|\; \tpushins \;|\; \\
			&  \ \ & \tconfirmedins{x} \;|\; \twhile{\cond}{\tprogram}  \;|\;\\
			&  \ \ & \tifte\\
      (\textsc{client}) & C \::=\ &  \tclient{\tprogram}{\tknown}{\tpending}{\ttransactionbuffer}{\tsent}{\treceivebuffer} 
       				\;|\; C\ \bigpar\ C \hfill \text{with}\  \tknown, \treceivebuffer\ \in \nat \\
			 (\textsc{system}) & N \::=\ & \tsystem{C}{\queuemessage} 
      \end{array}
  \]
\end{definition}

A global store $\queuemessage$ is represented by a sequence of update blocks.
Programs $P$ run by clients are written in a simple sequential language enriched with the primitives 
$\readtran{r},  \updatetran{u}, \pulltran ,  \pushtran, \confirmedtran$
for interacting with 
the store (we postpone their description until \secref{sec:gsp-sos}). Conditional and loop statements contain expressions 
$\cond, \cond', \ldots$ written in some unspecified language. As standard, we assume  
expressions to be equipped with an evaluation function, which is written $\eval{\_}$.
A program $\plet{x}{\ldots}{P}$ introduces a bound variable whose  scope is $P$. The definition of 
free variables of a programs is as usual. We say a process $P$ is {\em closed} when it has 
no free variables.


The term $\tclienti{\tprogram}{\tknown}{\tpending}{\ttransactionbuffer}{\tsent}{\treceivebuffer}$ 
stands for a client identified by $\cid$ that is engaged in the execution of  program $P$. 
...
store that is replicated locally, i.e., $S[0..k]$; and $j$ stands for the number of new updates 
in the global store that have been received and needs to be added to the local replica, i.e., $S[k+1..k+j]$.
A system is composed by a store and a non-empty set of clients. 

A system $N = S \;|\ C_0 \;|\ \ldots \;|\  C_n$  with $C_l = \tclienti{\tprogram_l}{\tknown_l}{\tpending_l}{\ttransactionbuffer_l}{\tsent_l}{\treceivebuffer_l}[\cid_l]$ for all $0\leq j \leq n$  is well-formed if it 
satisfies the following conditions: 
\begin{itemize}
  \item $\cid_l \neq \cid_k$ for all $j\neq k$,   
  \item $k+j\leq \length S$
  \item $\cid = \cid_l$, for all $\ublock \in {\tpending_l}\cdot {\tsent_l}$
  \item $\cid \in {\cid_0,\ldots, \cid_n} $ for all $\ublock \in S$.
\end{itemize}
\changed{}{
...
\henote{we probably need some well-formedness condition requiring all clients to have different ids}

\subsection{Operational Semantics}
\label{sec:gsp-sos}
\changed{}
{\paragraph{Interpretation of read and update operations}
we need to explain rvalue}

We assume the following countable sets of vertex names $\verticesets$ ranged over by $v$;$v_0$;$v_1$, \ldots;
 ranged over by $u^v$,$u^{v_0}_0$,$u^{v_1}_1$,$\ldots$; Let \changed{$\mathcal{U}$=$u^{v_0}_0$ $\cdot$ $u^{v_1}_1$ $\cdot$,$\dotsc$,$\cdot$ $u^{v_n}_n$}{$\mathcal{U}$=$u^{v_0}_0$ $\cdot$ $u^{v_1}_1$ $\cdot$ $\dotsc$ $\cdot$ $u^{v_n}_n$}. For the sake of clarity, we shall only use vertices as update's decoration whenever it would be necessary.


The operational semantics of \tgspcalculus\ is defined by a labeled transition system 
over well-formed terms, up-to the structural congruence \changed{}{defined as the least 
congruence satisfying the following rules:
\[
  \begin{array}{l}
  TODO
  \end{array}
...
\changed{The labeled transition system considers the following actions:}{We rely on a set of labels defined by the following grammar:
}
\[ 
\begin{array}{r@{\ ::= \ }l}
...
\mathrule{t-while-false}{\condition{\cond}{true}}{\tsystem{\tclienti{\twhile{\cond}{\tprogram;Q}}{\tknown}{\tpending}{\ttransactionbuffer}{\tsent}{\treceivebuffer} \bigpar \system}{\queuemessage} \arro{\tau} \tsystem{\tclienti{Q}{\tknown}{\tpending}{\ttransactionbuffer}{\tsent}{\treceivebuffer+1} \bigpar\ \system}{\queuemessage}}
...
\end{figure*}
 ...
 \changed{\tgspcalculus\ program is given by a Labeled Transition System (LTS) before introducing. Transitions for systems are labeled by actions $\alpha$ as usual}{The operational semantics of a \tgspcalculus\ system is given 
 by the inference rules in \figref{fig:OS-tgsp}}.
Rule $\textsc{(t-read)}$ states the behaviour of the client $i$  performing a reading action  \changed{$\readtran{r}$} {$r$}. 
\henote{$i$ is not defined in the label, nor in the arrow}.

\changed{}{Such operation $r$ is evaluated over the client's local view of the global state, that is, 
the known part of the server state $S[0..\tknown]$ and the  uncommitted operations in the  internal queues 
of the client $\tpending$ and $\ttransactionbuffer$. The obtained value $v$ is bound to the variable $x$, and hence $x$ is 
substituted by $v$ in the continuation $P$ of the program}.

\henote{it could be a good idea to move t-read after the other rules for presentation purposes.}

Rule $\textsc{(t-update)}$ describes effects performed by an update operation. The update $u$ is saving in the temporal queue $\ttransactionbuffer$. After $\textsc{(t-push)}$ the content of the transaction queue $\ttransactionbuffer$ is moved to the pending queue, $\tpending$, and the sent queue, $\tsent$. Rule $\textsc{(t-pull)}$ increases the amount of updates that client $i$ can read in the message queue $S$. This increase is given by the amount of update receiving.The pending queue removes those updates which belong between index $\tknown$ and the amount of update receiving $\treceivebuffer$ in $S$. Besides, $\treceivebuffer$ is restarted in 0. $\textsc{(t-confirmed)}$ sets $x$ in false whether updates are in pending queue or transaction queue, true in otherwise. Rule $\textsc{(t-while-trye)}$ and $\textsc{(t-while-false)}$ are standard. Rule $\textsc{(t-receive)}$ states that a counter $\treceivebuffer$ is incremented (by 1). Finally, the last one rule, $\textsc{(t-process)}$, moves updates from sent queue to the message queue.  
...
Let $\mathcal{N}$ be a set of clients' name ranged over by $b$, $b_1$, $b_2$, $\ldots$; \gsprefixtype\ be a set over sets \statetype\ $\times$ $\mathcal{N}$ $\rightarrow$ $\mathbb{N}$, \gssegmenttype\ be a set operation over sets \deltatype $\times$ $\mathcal{N}$ $\rightarrow$ $\mathbb{N}$ and \roundtype\ the set over sets $\mathcal{N}$ $\times$ $\mathbb{N}$ $\times$ \deltatype; $\emptygssegment$\ is a fresh element of \gssegmenttype.
...
\mathrule{batch}{rs=\receiveroundsname{\inserver} \qquad gs=\append{\emptygssegment}{rs} \qquad ps'=\applyplus{\persistedstate}{gs.delta}}{\server{\persistedstate}{\inserver}{\outserver} \arro{\tau} \server{ps'}{\inserver}{\notify{dom(\outserver)}{\outserver}{gs}}}
...
\mathrule{while-true}{\condition{\cond}{true}}{\clientr{\twhile{\cond}{\tprogram}}{\stateclient} \arro{\tau} \clientr{\tprogram;\twhile{\cond}{\tprogram}}{\stateclient}}

\\[25pt]

\mathrule{while-false}{\condition{\cond}{true}}{\clientr{\twhile{\cond}{\tprogram;Q}}{\stateclient} \arro{\tau} \clientr{Q}{\stateclient}}
