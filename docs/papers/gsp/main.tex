...
We  use $\ttransactionbuffer$, $\beta$ to denote sequences of decorate updates and
...
Formally,
\[ \ttransactionbuffer, \beta := \epsilon \ |\ \udec\cdot\alpha 
    \qquad\qquad 
...
\begin{itemize}
    ...
    %relative complement of $\tpending$ in $\ttransactionbuffer$, \ie,
     $\tpending \setminus \sigma = [\ublock | u_i \in \tpending \wedge\ u_i\notin \ttransactionbuffer]$. 
     \henote{fix}
...
store that is replicated locally, i.e., $S[0..k-1]$; and $j$ stands for the number of new updates 
in the global store that have been received and needs to be added to the local replica, i.e., $S[k..k+j-1]$.
A system $N$ is composed by a store and a  set of clients. 
...
store $S[k..k+j-1]$ are removed from  $\tpending$. 
% 
...
the known part of the server state $S[0..\tknown-1]$ and the local updates in 
...
The \tgspcalculus\ model formalised in the previous section corresponds to an idealised model that
abstracts away from important implementation details, mainly, the  problem of maintaining a constantly growing 
sequence of updates to represent store states.  A realistic implementation for such model has been 
proposed in\henote{~\cite{}}, in which states and changes have a compact representation in 
...
\gsprefixtype\ be a set over sets \statetype\ $\times$ $\idset$ $\rightarrow$ $\mathbb{N}$, \gssegmenttype\ be a set operation over sets \deltatype $\times$ $\idset$ $\rightarrow$ $\mathbb{N}$ and \roundtype\ the set over sets $\idset$ $\times$ $\mathbb{N}$ $\times$ \deltatype; $\emptygssegment$\ is a fresh element of \gssegmenttype.
...
Let $\inserver$\ be a partial function in $\partialfunction{\idset}{\dominserver}$

Let $\outserver$\ be a partial function in $\partialfunction{\idset}{\gssegmenttype^* \cup \gsprefixtype}$ \\
...
\subsection{Syntax}
 \begin{definition}[Implementation of GSP] 
  ...
  \[
    \begin{array}{l@{\quad}r@{\;::=\;}l}
	(\textsc{system}) & \systemterm &  S\ \bigpar\ C \\
	(\textsc{server}) & S & \server{\persistedstate}{\inserver}{\outserver} \\
	(\textsc{clients}) & C & \zero \;|\; \client{P}{<\statetype,\pendingtype,\pushbuffertype,\transactionbuffertype,\receivebuffertype,\nroundtype,\inclient,\outclient>} \;|\; C\ \bigpar\ C  \\
...
\begin{flushleft}
\specOperator{notify}{$\idset^*$}{($\partialfunction{\idset}{\gssegmenttype^* \cup \gsprefixtype}$)}{\gssegmenttype}\\
