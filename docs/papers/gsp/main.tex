...
then for all $\vertice\in\dom\op$ if $\op(\vertice) = u$ \henote{and $\vertice\in\sse(\cid)$} then
...
% u an update action, $\queuemessage$ a message queue, $\tpending_i$ and
 % $\ttransactionbuffer_i$ a pending queue and transaction queue from the client i, if $\environment{\systemterm}{\emptyset}{\emptyset}{\emptyset}{\emptyset} \arro{} ^*\ \environment{\systemterm}{\op}{\so}{\vis}{\arb}$ then $\{x \mapsto \updatebyclient{i} \}\ \in \ \op\ \Rightarrow\ 
and there exists $\anupd$ s.t. $\udec \in\  \queuemessage[0..\tknown_{\cidj}-1]$.


\end{lemma}

\begin{proof} \henote{To Do}
\end{proof}
...
By \thmref{theorem:read-my-writes},
$\restUR{\so}  \subseteq \vis$, hence  $(\so \cup \vis ) ^+ \subseteq \vis^+$.  
We now prove that $\vis^+$ is acyclic, which implies  $(\so \cup \vis ) ^+$.
Actually, it suffices to prove that $\vis$ is acyclic because the transitive closure of an 
acyclic graph is a strict partial order.

This can be shown by  induction on the length of the derivation 
$\environment{\systemterm}{\emptyset}{\emptyset}{\emptyset}{\emptyset}[\emptyset] \tr{} ^*\ \environment{\systemterm'}{\op}{\so}{\vis}{\arb}$.

The only interesting case corresponds to the application of rule  \ruleName{a-read},
which modifies $\vis$. Note that all the added pairs 
 have as a second component the same fresh vertex. Consequently, no cycle can be 
 introduced to $\vis'$. Then, $\vis$ is acyclic since $\vis'$ is acyclic by inductive hypothesis.  
\end{proof}
...
\begin{proof}
	We show by induction on $n$   that 
	   \[\restUO{(\so \cup \vis ) ^{+}}  = \bigcup_{n=1}^{\infty} \restUO{(\so \cup \vis )^{n}} \subseteq  \so  \cup \vis\]

	\begin{itemize}
	    \item {\bf n=1}. Trivially, because ${(\so \cup \vis )} \subseteq  (\so  \cup \vis)$.
	    \item {\bf n=k+1}. By definition of composition, 
	    ${(\so \cup \vis ) ^{n} }  = {(\so \cup \vis ) ^{k};(\so \cup \vis )}$.
	    \[(\so \cup \vis ) ^{k} = \restUO{(\so \cup \vis ) ^{k}} \cup\ \restRO{(\so \cup \vis)^k}\]
	    
	    Then, 
	    \[\restUO{((\so \cup \vis ) ^{k};(\so\cup\vis))} = \restUO{(\restUO{(\so \cup \vis ) ^{k}} ;(\so\cup\vis))}\]
	  ...
	  because    
	    ...
	    \[(\so \cup \vis ) ^{k}\subseteq (\so\cup\vis)\]
	    Hence, 
	    \[ {(\so \cup \vis ) ^{k};(\so \cup \vis )}\subseteq (\so\cup\vis);(\so\cup\vis)\]
	    By definition,
	    \[(\so\cup\vis);(\so\cup\vis) =  \so;\so\ \cup\ \vis;\so\ \cup\ \vis;\so\ \cup\ \vis;\vis\]
