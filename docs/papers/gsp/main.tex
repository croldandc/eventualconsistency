...
Consistency guarantees of models are usually characterised in terms of abstract executions~\cite{}, defined in terms of 
relations different relations between read and write events.  In particular, we will use the following relations:
...
\begin{itemize}
 \item {\bf Session Order}: it relates pairs of events corresponding to operations performed by the same client, indicating the program order. 
  This is a total order over the events of the same client. 

 \item{\bf Visibility}: it   relates updates and reads. It is used to indicate is if an action of Update is visible for an action of Read.

 \item{\bf Arbitration}: it  Updates with Updates. It is used to resolve update conflicts. It is a total order on actions of update.
\end{itemize}

We know provide an operational approah to associate abstract executions to systems.
