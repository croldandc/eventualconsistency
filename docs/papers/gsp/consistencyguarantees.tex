...
In this section we analyse the properties 
...
is well-formed if the following conditions hold:
 ...
 \begin{enumerate}
     ...
     \item $\arb\subseteq\mathbb{U}\times\mathbb{U}$.%, then $\{\op(\vertice[@][_1]),\op(\vertice[@][_2])\}\subseteq\updatesets$;
     \item $\arb$ is an order?
 \end{enumerate}
\end{definition}
...
the definition in~\cite{}, we restrict visibility to keep track of dependencies between updates and read events(4), while 
arbitration orders update events (5).  
...
the global state $\queuemessage[0..\tknown_{\,\cid}-1]$ and the local temporary buffers 
$\tpending_{\cid}$ and $\ttransactionbuffer_{\,\cid}$. Rule \ruleName{a-arb} handles the changes in the 
state of the global store (due to a send transition in one client) and just amends $\arb$ by (i) arbitrating the 
...
We use abstract histories to analyse the ordering guarantees 
offered by the \gsp\ model. We focused on the hierarchy in~\cite{DBLP:journals/ftpl/Burckhardt14}. That are listed in ~\figref{}.
