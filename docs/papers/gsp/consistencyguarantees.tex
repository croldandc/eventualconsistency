...
case analysis on the rule applied for the last transition:
	...
	\begin{itemize}
       ...
       \item rule \ruleName{a-arb}. Then, $\queuemessage' = \queuemessage''\cdot\udec$, $\so = \so'$, 
...
\label{lem:server-reflect-arb}
 Let $\environment{\systemterm}{\emptyset}{\emptyset}{\emptyset}{\emptyset}[\emptyset] \tr{} ^*\ \environment{\systemterm'}{\op}{\so}{\vis}{\arb}$.
...
case analysis on the inference rule applied for the last transition:
	...
	By inductive hypothesis,    $\arb';(\vis'\setminus\sse) \subseteq \vis'= \vis$. It remains
       ...
       \item rule \ruleName{a-arb}. Then, $\queuemessage'= \queuemessage''\cdot\udec$, $\vis = \vis'$, $\sse = \sse'$  and
        $\arb = \arb' \cup \arb_{\mbox{\sf new}}$ with 
         $\arb_{\mbox{\sf new}}= \{ (\vertice, \vertice[w])\ \ |\ \  \vertice[w]\in\dom{\op}, \ \op(\vertice[w])\in\updatesets, \
		     \forall\anupd_0. \anupd_0^{\vertice[w]}\not\in\queuemessage\cdot \udec\}$.
	...
	By inductive hypothesis,    $\arb';(\vis'\setminus\sse) \subseteq \vis'= \vis$. It remains
        ...
        \exists\anupd_0. \anupd_0^{\vertice[y]}\in \queuemessage''\cdot\udec
        \end{equation}
...
$   \Absclient[_1]''  =  {\anabstcli[{}][Q\subst{y}{v'}][0][\epsilon][{\anupd_1}^{\vertice[v][_1]}][\epsilon][0][0]}_{\cid_1}$ and
 ...
 Since $\iread{r_1}{{\anupd_1}^{\vertice[v][_1]}}=v$ and $\iread{r_2}{{\anupd_2}^{\vertice[v][_2]}}=v'$, 
  none of the clients read the update performed by the other.
