...
$\anupd_1^{\vertice[w]}\in\queuemessage[0..m-1]$.
       ...
       $ \anupd_0^{\vertice[y]}\not\in\queuemessage''$. 
       Therefore, it holds.
       \item $(\vertice,\vertice[y]) \in \{(\vertice[@][_i],\vertice[@][_j]) \ |\ \vertice[@][_i],\vertice[@][_j]\in \{\vertice[@][_0],\ldots,\vertice[@][_n]\}, i<j\} $ follow immediately by \lemref{lemma:arSO-in-arUso}. \chnote{esto no sé si es suficiente justificacion. El hecho de estar en el session order significa q hay un orden entre los vertices que estaban en los buffer}
\end{itemize}
