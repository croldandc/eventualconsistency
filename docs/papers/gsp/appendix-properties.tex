...
will be useful when proving the main result of the section.

\subsection{Results relating system states and abstract executions}
...
\begin{itemize}
   \item{\bf n=0}. It holds, because $\tpending = \epsilon$ and $[\ttransactionbuffer] = \epsilon$ and $\op=\emptyset$.
    ...
    By inductive hypothesis, for all $m < n < |\flatten{\tpending_{\cid}} \cdot \ttransactionbuffer_{\cid}|$ if $\anupd[^{\vertice[w]}_0]=\flatten{\tpending_{\cid}'} \cdot \ttransactionbuffer_{\cid}'[m]$ {and $\anupd[^{\vertice[w']}_1]=\flatten{\tpending_{\cid}'} \cdot \ttransactionbuffer_{\cid}'[n]$}  then
...
\qed
	\end{itemize}
%\chnote{dentro de las internas, el moviemiento que se genera por la push me parece trivial e inmediato, por eso no lo separé}
\end{itemize}
\end{proof}
...
case analysis on the inference rule applied for the last transition:
	...
	and     
        ...
        the reduction $\systemterm''\arro{\tau}[{\cidj}]\systemterm'$
            \begin{itemize}
		 \item  rule \ruleName {pull}.  Then,  $\tknown_{\cidj} = \tknown_{\cidj}'+\treceivebuffer_{\cidj}'$, while  $\tknown_{\cid} = \tknown_{\cid}'$
		for all $\cid \neq \cidj$.
		By inductive hypothesis, there exists $\cid$ s.t.
	...
	then there exists $\anupd_1$ s.t. $\anupd_1^{\vertice}\in\queuemessage'[0..k_{\cid}^{'}-1]$.
		 ...
		 When $\cid = \cidj$. Assume  
		 $\anupd_0^{\vertice[w]} \in\  \queuemessage[0..\tknown'_{\cidj}]$. Then,  by \lemref{lemma:update-ever-belong} 
		 $\anupd_1^{\vertice} \in\  \queuemessage[0..\tknown_{\cid}-1] \cdot \tpending_{\cid} \cdot [\ttransactionbuffer_{\cid}]$.
		 If $\anupd_1^{\vertice} \in\  \queuemessage[0..\tknown_{\cid}-1]$, then we are done. Otherwise,
		  if $\anupd_1^{\vertice} \in \tpending_{\cid} \cdot [\ttransactionbuffer_{\cid}]$, we conclude that 
		  $\anupd_1^{\vertice}\in\queuemessage[0..\tknown_{\cid}+\treceivebuffer_{\cid}-1]$ by \lemref{lemma:upd-in-buffers-updinSO}.
...
$\anupd_1^{\vertice[w]}\in\queuemessage[0..m-1]$.
       ...
       $ \anupd_0^{\vertice[y]}\not\in\queuemessage''$. 
       Therefore, it holds.
       \item $(\vertice,\vertice[y]) \in \{(\vertice[@][_i],\vertice[@][_j]) \ |\ \vertice[@][_i],\vertice[@][_j]\in \{\vertice[@][_0],\ldots,\vertice[@][_n]\}, i<j\} $ follow immediately by \lemref{lemma:upd-in-buffers-updinSO}. %\chnote{esto no sé si es suficiente justificacion. El hecho de estar en el session order significa q hay un orden entre los vertices que estaban en los buffer}
\end{itemize}
...
\subsection{Properties of relations in abstract executions}
...
Let $\systemterm$ a \gsp\ system. For all  $\systemterm[']$, if 
\[\environment{\systemterm}{\emptyset}{\emptyset}{\emptyset}{\emptyset}[\emptyset] \tr{} ^*\ \environment{\systemterm[']}{\op}{\so}{\vis}{\arb}\]
then $\so\relrestriccion{\sse(\cid)}$ is a total order for all $\cid$.
\end{lemma}
...
\begin{itemize}
   \item{\bf n=0}. It holds trivially, because $\so=\emptyset$.
    ...
    \tr{} ^k\ \environment{\systemterm''}{\op'}{\so'}{\vis'}{\arb'}[\sse'] 
    \arroi{\lambda} \environment{\systemterm'}{\op}{\so}{\vis}{\arb}[\sse].\] 
    By inductive hypothesis, $\so'\relrestriccion{\sse'(\cid)}$  is transitive for all $\cid$.
    We proceed by 
    case analysis on the last applied rule:
	
    \begin{itemize}
	\item rule \ruleName{a-update} and \ruleName{a-read}. Then, $\so = \so'\cup (\sse'(\cid)\times\{\vertice\})$ for all $\cid$. It is 
	straightforward to notice that $\so\relrestriccion{\sse(\cid)}$ is the total order obtained by extending $\so'\relrestriccion{\sse'(\cid)}$ 
	 with the maximal element $\{\vertice\}$.
...
\begin{proof}
It follows induction on the length of the derivation  $\environment{\systemterm}{\emptyset}{\emptyset}{\emptyset}{\emptyset}[\emptyset] \tr{} ^n\ \environment{\systemterm'}{\op}{\so}{\vis}{\arb}$. Inductive step follows by case analysis on the last applied rule. Rules that change the
relations $\so$ and $\vis$ are  \ruleName{a-update} and  \ruleName{a-read}. In both cases, all new pairs in 
$(\so \cup \vis )$ are of the form $(\vertice,\vertice[w])$ with $\vertice[w]$ fresh. Hence, no cycles can be introduced.
...
case analysis on the rule applied for the last transition:
	...
	\begin{itemize}
       ...
       \item rule \ruleName{a-arb}. Then, $\queuemessage = \queuemessage'\cdot\ublock[{\udec[{\anupd[_0]}][{\vertice[@][_0]}]}\cdots{\udec[{\anupd[_n]}][{\vertice[@][_n]}]}]$, $\so = \so'$, and $\arb = \arb' \cup \arb_{\mbox{\sf new}}$ with 
         \[
         \begin{array}{l}
         \arb_{\mbox{\sf new}}= \{(\vertice[@][_i],\vertice[@][_j]) \ |\ i,j\in \{0,\ldots,n\}, i<j\} \ \cup\hspace{3.7cm}{ \ }\\
         \hfill
...
\subsection{Proof of Lemma \ref{lemma:prop-wf} (Well-formedness of Abstract Histories)}
The proof follows by case analysis on the applied rule. Conditions 1-4 follows straightforwardly by
rule inspection. The remaining one are as follows
\begin{itemize}
   \item[5.] The interesting cases are rules 
      \begin{itemize}
      	\item \ruleName{a-update}: $\arb$ is a prefix order. Moreover, condition 6 ensures that for all $\vertice,\vertice[v']$ s.t.
	$\anupd_0^{\vertice}\in\queuemessage$ and $\anupd_1^{\vertice[v']}\in\queuemessage$, either 
	$(\vertice,\vertice[v']) \in \arb $ or  $(\vertice[v'],\vertice) \in \arb$. Therefore, $\arb'$ is a prefix order.
	
	 \item \ruleName{a-arb}: $\arb$ is a prefix order. Note that the set
	 	$$\{(\vertice[@][_i],\vertice[@][_j]) \ |\ i,j\in \{0,\ldots,n\}, i<j\}$$
		 is a total order. Moreover, condition 6 (ii) ensures that 
		for all $\vertice[@][_i], \vertice[@][_j]$ s.t. $i,j\in \{0,\ldots,n\}$,  $(\vertice[w],\vertice[@][_i]) \in \arb$ implies  $(\vertice[w],\vertice[@][_j]) \in \arb$.
		Then it satisfies transitivity and downward totality. Additionally, condition 6 (ii) ensures that 
		for all $\vertice[@][_i]$ s.t. $i\in \{0,\ldots,n\}$,  there does not exist $\vertice[w]$ s.t. $(\vertice[@][_i], \vertice[w]) \in \arb$, hence, 
		it satisfies  antisymmetry. 
		
		Then,  $\arb\cup\{(\vertice[@][_i],\vertice[@][_j]) \ |\ i,j\in \{0,\ldots,n\}, i<j\}$ is a prefix order. Reasoning analogously, 
		for  
		$\ (\{\vertice[@][_0],\ldots,\vertice[@][_n]\}\times 
		\{ \vertice[w]\ \ |\ \  \vertice[w]\in\mathbb{U}, \
		     \forall\anupd. \anupd^{\vertice[w]}\not\in\queuemessage\cdot\anupdseq\})
		 $   
		 it can be concluded that $\arb'$ is a prefix order. 
     \end{itemize}
\item[6.] The interesting cases are  
      \begin{itemize}
      	\item \ruleName{a-update}:  It is immediate to conclude that the new pairs in  
	$$(\{ \vertice[w] \ |\ \udec[@][{\vertice[w]}]\in \queuemessage \}\times\{\vertice\})$$
	satisfy condition $6(ii)$
	
	
	 \item \ruleName{a-arb}: The items in 
	 	$$\{(\vertice[@][_i],\vertice[@][_j]) \ |\ i,j\in \{0,\ldots,n\}, i<j\}$$
		 satisfy condition $6(i)$. Moreover, 
		 pairs in 
		$$\ (\{\vertice[@][_0],\ldots,\vertice[@][_n]\}\times 
		\{ \vertice[w]\ \ |\ \  \vertice[w]\in\mathbb{U}, \
		     \forall\anupd. \anupd^{\vertice[w]}\not\in\queuemessage\cdot\anupdseq\})
		 $$   
		 satisfy condition $6(ii)$ because for all $\{\vertice[@][_i]\} \in \{0,\ldots,n\}$ there exist
		  $\anupd_0^{\vertice}\in\queuemessage$. 
		 
\item[7.] 		 Its a direct consequence of \lemref{lemma:upd-in-buffers-updinSO}.
     \end{itemize}
...
\subsubsection{Proof of Read My Writes (\ref{theorem:read-my-writes}).}
The proof follows by induction on the length of the derivation $\tr{} ^*$.
\begin{itemize}
...
\subsubsection{Proof of Monotonic Read (\ref{theorem:monotonic-read}).}
 The proof follows by induction on the length of the derivation $\tr{} ^*$.
\begin{itemize}
   \item{\bf n=0}. It holds trivially, because $\op=\vis=\emptyset$.
   \item{\bf n=k+1}. 
...
\subsubsection{Proof of No Circular Causality (\ref{thm:no-circular-causality}).}
...
\subsubsection{Proof of Causal Visibility (\ref{thm:causal-visibility}).}
...
\subsubsection{Proof of Causal Arbitration (\ref{thm:causal-arbitration}).}
...
\begin{itemize}
	...
	By using inductive hypothesis, 
      ...
      \[\restUU{(\so;\so)} \subseteq \restUU{\so}\]
       because of \lemref{lem:so-transitive}
...
\subsubsection{Proof of Consistent Prefix (\ref{thm:consistent-prefix}).}
