...
choice do not affect the results presented in this paper. Actually, we could just  have  characterised 
...
A  label  $(\lambda,\cid)$ indicates that  
 the client $\cid$ performs the action $\lambda$. As usual,  $\tau$ stands for an internal, 
 unobservable action, while the remaining ones allow 
a client to update, read from and synchronise with the global store. 
In what follows we will write  $\arro{\mu}$ instead of $\tr{(\mu,\cid)}$. 
