...
\begin{enumerate}
  ...
  \item $\cid_l\in\dom\outserver$ and if $\inclient_l\cdot\outserver \neq\epsilon$ then either
	\begin{itemize}
	...
	\end{itemize}
	and $\amxrf[_j](\cid_l) \leq \ \amxrf[_h](\cid_l)$ for all  ${\sf j}\leq{\sf j}$.
...
If a client $\cid$ is connected then $\cid\in\dom\inserver$ and  $\cid\in\dom\outserver$ 
(2). Besides, when a client is disconnected (\ie, $\cid_\in\dom\outserver$)  its input buffer is
empty (3). Last condition imposes that a segment of the form $\agssegpair$ can be sent only  as the first message in the flow
and that round confirmations $\amxrf[_j]$ appear in increasing order. 

In order to keep our  definition simple, we do not impose any 
condition about the coherence about the state of each replica and the 
global store.
...
\[\begin{array}{ll}
 ...
 %  \begin{itemize}
     ...
     \\
     &
     \dom{\amxrf[']} = \dom{\inserver} \ \mbox{and}\  
       \forall\cid_l\in\dom\inserver.\amxrf['](\cid_l) = \irounds^{k_l}_l
    \end{array}
 \] 
%\end{itemize}
...
\begin{lemma} If $\isystemterm$ is well-formed and $\isystemterm\arroi{\action}\isystemterm[']$, then $\isystemterm[']$ is well-formed.
\end{lemma}
