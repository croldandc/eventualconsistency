...
A client terminates a block by executing $\pushcmd$ (rule \ruleName{i-push}). At this time, the block $\transactionbuffer$ 
 ...
 available blocks are sent in the same round $\around = \aroundtuple[@][@][\pushbuffer]$, which is labelled with 
  ...
  The new round $\around$ is added at the end of the input buffer of the server corresponding to $\cid$, \ie, $\inserver(\cid)$ 
  ...
  global store:  $\aseg$ is removed from the top of the output channel $\outserver(\cid)$ and
   ...
   the local replica when $\cid$ performs $\pullcmd$. Rule \ruleName{i-pull-1} handles the normal 
   execution in which communication with the global store has not been reset. In such case, 
...
The remaining rules handle deal  connectivity issues.     
