...
Condition (4) states that $\agsprefpair$ can appear only as the first message in the flow from 
the store and that the store confirms processed rounds in a non-decreasing order. Similarly, 
 clients send rounds with increasing round number (5). The last condition states a 
...
A client terminates a block by executing $\pushcmd$ \ruleName{i-push}. At this time, the block $\transactionbuffer$ 
 ...
 This transition may take place whenever  the client is connected  (\ie, $\cid\in\dom\inserver$),
  there are finished blocks in $\pushbuffer$ (\ie, $\pushbuffer \neq \emptydelta$)  and the 
  client does not need to re-synchronise with the global store 
 ...
 by the implementation.}.
  %
 The 
 available blocks are sent within the same round $\around = \aroundtuple[@][@][\pushbuffer]$, which is labelled with 
 the number  $\irounds$ corresponding to the last finished block. 
  ...
  on the global store and
   ...
   all rounds that are confirmed by the received segments are removed from  the  pending list $\pending$.
   Actually, it suffices to consider  
    the confirmation $ \amxrf_k$ since  well-formedness (\defref{def:wf-igsp},\ref{wf-igsp-pending}) ensures  
    $\amxrf_h(\cid) \leq \amxrf_j(\cid)$ for all ${h}\leq {j}$. Hence, all rounds up-to  
