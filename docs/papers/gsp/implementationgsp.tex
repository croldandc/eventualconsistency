...
interpretation function for read operations (\ie, the implementation counterpart of 
...
a function $\amxrf$ that keeps track of last rounds incorporated
 to the state, and the communication buffers
...
\begin{enumerate}
  ...
  \item if $\cid_l\in\dom\outserver$ and $\inclient_l\cdot\outserver(\cid_l) \neq\epsilon$ then either
	...
	\end{itemize}
	 $\amxrf_j(\cid_l) \leq \ \amxrf_k(\cid_l)$ for all  ${ j}<{k}$ and $\amxrf_h = \amxrf$
	
\item \label{wf-igsp-pending} $\pending_l = \aroundtuplei[0]\cdots\aroundtuplei[r]$,  $\irounds_j < \irounds_k$ for all  ${ j}<{k}$ and either.
	\begin{itemize}
...
the store and that store confirms processed rounds in a non-decreasing order. Similarly, (5)
...
A client terminates a block by executing $\pushcmd$ (rule \textsc{i-push}). At this time, the block $\transactionbuffer$ 
