...
\[\mu := \ldots\ | \ \tau \]
The label $\tau$ without any client annotation is associated with changes in the global store 
and communication failures.

As for the idealised model, the operational semantics  is given by a labelled transition system 
over well-formed terms, up-to  structural equivalence, which states that  $\bigpar$ is associative, 
...
which builds a unique object $\adelta$ by appending all  $\adelta$'s contained in the rounds in
input buffers, and 
...
$\cid$: the buffers $\outserver(\cid)$ and $\inserver(\cid)$ are removed from the global store and also 
the input buffer of $\cid$ is set to $\epsilon$. When the client $\cid$ (re-)establishes its connection, the store
...
do not model this behaviour explicitly because  it can be obtained by applying several times rule 
\ruleName{i-drop-cxn}.
%
%\\[25pt]
