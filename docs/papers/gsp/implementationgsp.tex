...
\item \label{wf-igsp-pending} $\pending_l = \aroundtuplei[0]\cdots\aroundtuplei[r]$,  $\irounds_j < \irounds_k$ for all  ${ j}\leq{k}$ and either.
	\begin{itemize}
...
(2). Moreover, the input buffer of a  disconnected is empty, \ie, $\cid_l\in\dom\outserver$ (3). 
Condition (4) states that $\agssegpair$ can appear only as a first message in the flow from 
the store and that store confirms processed rounds in an increasing order. Similarly, (5)
states that each client sends rounds with increasing number and that the last sent round
cannot be greater than the round counter of the client. The last condition states a 
 coherence requirement between  
the pending rounds on the client and the confirmations sent by the store.
...
A client terminates a block by executing $\pushcmd$ (rule \ruleName{i-push}). At this time, the block $\transactionbuffer$ 
 ...
 This transition may take place whenever  the client is connected  (\ie, $\cid\in\dom\inserver$) to and 
 synchronised with (\ie, $\inclient \cdot\outserver(\cid) \neq \agssegpair[{\astate[']}][{\amxrf}]\cdot\aseqseg$)\footnote{For simplicity we check re-synchronisation by inspecting  communication buffers  instead of 
 ...
 available blocks are sent in the same round $\around = \aroundtuple[@][@][\pushbuffer]$, which is labelled with 
  ...
  global store:  $\aseg$ is removed from the top of the output channel $\outserver(\cid)$ and
   ...
   the  pending rounds in $\pending$ are updated by removing the  local changes 
    that are confirmed by the received segments. By well-formedness (\defref{def:wf-igsp},\ref{wf-igsp-pending}),  
    $\amxrf_h(\cid) \leq \amxrf_j(\cid)$ for all ${h}\leq {j}$, it suffices to consider  
    the confirmation $ \amxrf_k$. Hence, all rounds up-to  
    $\adelta_k(\cid)$ are removed from $\pending$. This is done by the auxiliary function 
...
Rules \ruleName{i-read} and \ruleName{i-confirm} are analogous the ones on the \gsp\ calculus.   We use
$\igetdeltas{\_}$ as the function that projects a sequence of rounds into a sequence containing the 
...
%\end{itemize}
   ...
   to  every connected client (i.e., $\agssegpair[@][{\amxrf{[\amxrf[']]}}]$ is added at the end of
    every buffer $\outserver(\cid))$. Every input buffer  $\inserver(\cid)$ is
...
$\cid$: the buffers $\outserver(\cid)$ and $\inserver(\cid)$ are eliminated from the global store and the 
...
The proposed implementation allows for a server to crash, i.e., to close all communication buffers. We
do not model this explicitly because  its effects can be obtained by applying several times rule 
\ruleName{i-drop-cxn}.
%
%\\[25pt]
