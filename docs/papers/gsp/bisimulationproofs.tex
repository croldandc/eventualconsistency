...
%						\item[\ref{prop_size_buffersent}.] It trivially holds since in both cases the counters are incremented by one. \chnote{Esta hipotesis sospechamos que sobra.}
					...
					\item[\ref{prop_deltas}.] Since  $\implements{\isystemterm}{\systemterm}$,  \refprop{prop_transactions} ensures 
					that 
					for all $\agssegpair\in\inclient\cdot\outserver(\cid)$, $\amxrf(\cid)<\irounds$. Consequently
				...
				Then, we take $\systemterm'=\systemterm$ and 							
				  ...
				  still hold for $\cid$.			
					...
					%\henote{estas usando $\delta_p$ en lugar de $\pushbuffer$, gran lio si cambiamos las macros}.
						  ...
						  {\propref{prop_sent}} ensures that 
  							 \begin{itemize}
	...
	\item {\bf rule \ruleName{i-pull${_1}$}}. Then, $\isystemterm  \arroi{\pulltran} \isystemterm '$ with 
		...
		%				$\iapply{\known}{\ireduce{\adelta[_1]\cdots\adelta[_n]}}\triangleleft \flatten{\queuemessage[0 .. \tknown+\treceivebuffer - 1]} \ $					\[
					...
					\item[\ref{prop_deltas}.] It follows immediately because of \refprop{prop_deltas} and the fact that 
						...
						$\ireduce{\igetdeltas{\filter{\amxrf[_k](\cid)}{\pending}}} 
							\triangleleft 
							\flatten{\tpending \setminus \queuemessage[\tknown .. \tknown + \treceivebuffer -1]}$.
					         By \refprop{prop_pending}, 
					         $\ireduce{\igetdeltas{\pending}\cdot\pushbuffer} \triangleleft \flatten{\tpending_l}$.
					         By \refprop{prop_deltas} for all $\agssegpair\in\inclient_l\cdot\outserver(\cid_l)$, 
           for all $\aroundtuple[@][@][\adelta'] \in\pending_l$
        if $\irounds\leq\amxrf(\cid_l)$ then $\adelta\triangleleft \queuemessage[x..x']$ and  $\adelta'\triangleleft \queuemessage[y..y']$
        with $y'\leq x'$
