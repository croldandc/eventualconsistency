...
\item {\bf rule \ruleName{update}}. Then, $\isystemterm  \arroi{\updatetranaux{\anupd^{\vertice[@][']}}} \isystemterm '$ with 
				\[\begin{array}{r@{=}l}
				...
				By inspection of rules in \figref{fig:OS-tgsp}, 
...
\item {\bf rule \ruleName{receive}}. Then, $\isystemterm  \arroi{\tau} \isystemterm '$ with 
				...
				Moreover, 
\henote{corregir				\[\systemterm = \tsystem{\tclienti{\tprogram}{\tknown}{\tpending}{\ttransactionbuffer}{\tsent}{\treceivebuffer}}[{\systemterm['']}] \]
			}
				with $\implements{\isystemterm['']}{\systemterm['']}$. 
				...
				\begin{itemize}
					...
					then $\ireduce{\adelta_{h'+1}\cdots\adelta_h}\triangleleft \flatten{\queuemessage[\tknown+\treceivebuffer..|\queuemessage| - 1]}$. 
					By \lemref{lemma:reducefirst},  
					$\adelta_{h'+1}\triangleleft \blockseq$, $\ireduce{\adelta_{h'+2}\cdots\adelta_h}\triangleleft \blockseq[']$
					and $ \flatten{\queuemessage[\tknown+\treceivebuffer..|\queuemessage| - 1]}= \blockseq\cdot\blockseq[']$.
					Therefore, there exists $m$ s.t. %$\tknown+\treceivebuffer \leq m\leq|\queuemessage|  - 1$ and
					 $\blockseq = \flatten{\queuemessage[\tknown+\treceivebuffer .. \tknown+\treceivebuffer +m - 1]}$ and
					with $m \leq |\queuemessage| - \tknown- \treceivebuffer$. Hence, 
				...
				We check now that  $\implements{\isystemterm[']}{\systemterm[']}$.	
				\begin{itemize}
	...
	\item {\bf rule \ruleName{pull-1}}. Then, $\isystemterm  \arroi{\pulltran} \isystemterm '$ with 
				...
				by checking the properties in \defref{def:implementation}.	
					...
					\item[\ref{prop_inclient}.] $\inclient$ = $\epsilon$ and $\treceivebuffer$ = 0, hence, $\ireduce{\epsilon} = \emptydelta$ and $\flatten{\queuemessage[\tknown .. \tknown - 1]} = \epsilon$ then It holds since $\medskip\emptydelta \triangleleft \epsilon$, by rule \ruleName{\triangemptydelta}.
							\item[\ref{prop_state_known}.] We know that $\known \triangleleft \flatten {\queuemessage[0..\tknown-1]}$ by \refprop{prop_state_known} and $\ireduce{\adelta[_1]\cdots\adelta[_n]}\triangleleft \queuemessage[\tknown ..  \tknown + \treceivebuffer - 1]$ by \refprop{prop_inclient} . Then, we produce the following proof for 
