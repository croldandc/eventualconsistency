...
The following result states properties about relation $\triangleleft$ that will be use in the proof of 
...
\item \label{lemma:storechanged} Let $\adelta[_0]\cdots\adelta[_{n-1}]$ a delta sequence, such that, $\adelta[_0] \triangleleft \anupdseq[_0] \ldots \adelta[_0] \triangleleft \anupdseq[_n]$ and  $\astate \triangleleft \anupdseq$. Then $\iapply{\astate}{\ireduce{\adelta[_0]\cdots\adelta[_n]}}\triangleleft \anupdseq \cdot \anupdseq[_0]\cdots\anupdseq[_n]$.
\item \label{lemma:reduceempty} $\ireduce{\epsilon} = \emptydelta$. 
\end{enumerate}
...
\begin{itemize}
\item[ \ref{lemma:apply}.]
 $\Rightarrow)$ The only possible derivation for $\iapply{\astate}{\ireduce{\adelta[_0]\cdots\adelta[_n]}}\triangleleft \anupdseq$ is
...
$\Leftarrow)$ It follows analogously. 
...
\item[\ref{lemma:reduce}.] $\Rightarrow)$ The only possible derivation for $\ireduce {\adelta[_1] \cdots \adelta[_{n-1}] \cdot \adelta[_{n}]} \triangleleft \anupdseq$ is
...
%\end{lemma}
%\item[\ref{lemma:reducefirst}.] Follows analogously to the previous case.
%
%\item[\ref{lemma:reduceinreduce}.] Follows analogously to  case \ref{lemma:reduce}.
%
%\item[\ref{lemma:storechanged}.] The proof follows by induction on the length of the sequence.
%
%\begin{itemize}
...
\subsection{Proof of~\thmref{thm:simulation}} 
...
\]
We proceed by case analysis on the applied rule for the derivation of $\isystemterm\tr{\lambda}\isystemterm[']$:
...
\item {\bf rule \ruleName{i-push}}. Then, $\isystemterm  \arroi{\pushtran} \isystemterm '$ with 
				...
				Since the computation involves only client $\cid$, we only need to check that  properties in \defref{def:implementation} still hold for $\cid$.		
...
\item {\bf rule \ruleName{receive}}. Then, $\isystemterm  \arroi{\tau} \isystemterm '$ with 
				...
				Since $\implements{\isystemterm}{\systemterm}$, \refprop{prop_inclient} holds. We have the following cases:
				\begin{enumerate}[(i)]
				       \item It cannot be that ${\inclient}_l \cdot \outserver(\cid_l)  = \epsilon$ because  $\ruleName{receive}$ has been applied.
				...
				are $\inclient$, $\outserver(\cid)$ and $\treceivebuffer$, we  only need to check that \refprop{prop_inclient} and \ref{prop_deltas} still hold for client $\cid$.
...
%
				...
				It follows analogously to the previous case, because \equref{eq:sim-receive-c2-d} is equal to  \equref{eq:sim-receive-c1-d} and 
				 \equref{eq:sim-receive-c2-c}  are  \equref{eq:sim-receive-c1-c} analogous.  The proof for \refprop{prop_inclient} holds 					
				by using \lemrefprop{lemma:sim-aux}{lemma:reduce} with the hypothesis \equref{eq:sim-receive-c2-c} and \equref{eq:sim-receive-c1-a}.
				As in the previous case, \refprop{prop_deltas} straightforwardly holds.
     		...
     		%$\astate['] = \iapply{\initialstate}{\ireduce{\adelta_1\cdots\adelta_{z}}}$,
               %$\astate['] \triangleleft \queuemessage[0 .. t - 1]$ and $t \leq \tknown_l+\treceivebuffer_l$; 
				...
				\item  ${\inclient} = \epsilon$, 
					$\outserver(\cid) = \agssegpair[{\astate[']}]\cdot \aseqseg$ then  there exists 
					$t \geq \tknown+\treceivebuffer$ s.t.
		     		        $\astate['] \triangleleft \ \flatten{\queuemessage[0 .. t - 1]}$ 
				        and       		$\ireduce{\igetdeltas{\aseqseg}}\triangleleft \ \flatten{ \queuemessage[t..|\queuemessage| - 1]}$.
				        Note that $\tknown+ \treceivebuffer \leq t \leq |\queuemessage|$
		
				Define $r = t - \tknown - \treceivebuffer$.  If $r = 0$ then take $\systemterm[']=\systemterm$.
				Otherwise, $r > 0$ implies
				$\tknown+ \treceivebuffer + r < |\queuemessage|$. Therefore,  $\systemterm$ can reduce   $r$  by repeatedly using rule
				\ruleName{receive}.  
				Consequently, $\systemterm \Arro{}\systemterm[']$ with 
				\[\begin{array}{r@{=}l}
						\systemterm['] & \tsystem{\tclienti{\tprogram}{\tknown}{\tpending}{\ttransactionbuffer}{\tsent}{\treceivebuffer+r}}[{\systemterm['']}]	
				  \end{array}		
				\]
				
				It is immediate to notice that \refprop{prop_inclient}($iii$) holds for $\systemterm[']$ and $\isystemterm[']$ because $t = \known+\treceivebuffer+r$.
				
				As in  case ($ii$), \refprop{prop_deltas} straightforwardly holds.
	...
	\item {\bf rule \ruleName{i-pull${_1}$}}. Then, $\isystemterm  \arroi{\pulltran} \isystemterm '$ with 
		...
		%				$\iapply{\known}{\ireduce{\adelta[_1]\cdots\adelta[_n]}}\triangleleft \flatten{\queuemessage[0 .. \tknown+\treceivebuffer - 1]} \ $					\[
					...
					$\flatten{\queuemessage[\tknown+\treceivebuffer .. \tknown+\treceivebuffer + 0 - 1]} = \epsilon$ and 
					 ...
					 Moreover, \refprop{prop_inclient} ensures
					 $\ireduce{\igetdeltas{\outserver(\cid)}}\triangleleft   
...
%					$\astate \triangleleft \flatten{\queuemessage}$
				...
				%by checking the properties in \defref{def:implementation}.			
				 The only term that has changed is the programs in client $\cid$.  We show that $v=v'$ holds. Since 
						$\implements{\isystemterm}{\systemterm}$, the following holds
						\begin{itemize}
						...
						\[
								  \mathrulean{\lemrefprop{lemma:sim-aux}{lemma:apply}}
									...
									\triangleleft {\flatten {\queuemessage[0..\tknown-1] \cdot \tpending} \cdot \ttransactionbuffer}}
					\]}
					Consequently, we conclude $v=v'$.% by \triangread.
...
\item {\bf rule \ruleName{i-confirm}}. Then, $\isystemterm  \arroi{\confirmedtran} \isystemterm '$ with 
		...
		{\tknown}{\tpending}{\ttransactionbuffer}{\tsent}{\treceivebuffer}}[{\systemterm['']}]	\\
				...
				We  prove that $(\isystemterm',\systemterm') \ \in \ \mathcal{R}$ by showing that $\implements{\isystemterm[']}{\systemterm[']}$ 
				by checking the properties in \defref{def:implementation}.
				The only terms that have changed are the programs in client $\cid$ we only need to show  $v=v'$. Since 
						$\implements{\isystemterm}{\systemterm}$, the following holds
						\begin{itemize}
							\item $\ireduce{\igetdeltas{\pending}\cdot{\pushbuffer}}\triangleleft \flatten{\tpending} \ $ by  \refprop{prop_pending};
							\item $\transactionbuffer \triangleleft{\ttransactionbuffer}$ by  \refprop{prop_transactions};
						\end{itemize}
						Since $\ireduce{\igetdeltas{\pending}\cdot{\pushbuffer}}\triangleleft \flatten{\tpending}$,  $\pending  \cdot \pushbuffer == \epsilon$ iff
						$\tpending==\epsilon$. Analogously,  $\transactionbuffer \triangleleft{\ttransactionbuffer}$  implies 
						 $ \transactionbuffer == \epsilon$ iff
						 $\ttransactionbuffer==\epsilon$. Therefore  $v = v'$.
...
%						\[\iapply{\astate[']}{\ireduce{\adelta[_1]\cdots\adelta[_k]}}\triangleleft \flatten{\queuemessage[0 .. \tknown +\treceivebuffer- 1]} \]
			...
			\item {\bf rule \ruleName{i-accept-cxn}}. Then, $\isystemterm  \arroi{\tau} \isystemterm '$ with 
				...
				\begin{enumerate}
					...
					\item[\ref{prop_inclient}.] We show that (iii) holds. By \propref{prop_stateserver}, we know that $\astate \triangleleft \queuemessage$. 
					Then, take $t = \queuemessage$ and notice that $\tknown+\treceivebuffer \leq|\queuemessage|$.
					\item[\ref{prop_deltas}.] It follows immediately by inductive hypothesis after noticing that $\amxrf = \amxrf[_s]$.
					\end{enumerate}
			...
			\item {\bf rule \ruleName{drop-conn}}. Then, $\isystemterm  \arroi{\tau} \isystemterm '$ with 
				...
				Then, take $\systemterm[']=\systemterm$ and  note that $\implements{\isystemterm[']}{\systemterm}$.
